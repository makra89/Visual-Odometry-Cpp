...
{
    cv::Mat1f allOnes = cv::Mat1f::ones(3, 4);
    ...
    // Second column
    EXPECT_FLOAT_EQ(intImage.GetIntensity(0, 1), 2);
    EXPECT_FLOAT_EQ(intImage.GetIntensity(1, 1), 4);
    EXPECT_FLOAT_EQ(intImage.GetIntensity(2, 1), 6);

    // Third column
    EXPECT_FLOAT_EQ(intImage.GetIntensity(0, 2), 3);
    EXPECT_FLOAT_EQ(intImage.GetIntensity(1, 2), 6);
    EXPECT_FLOAT_EQ(intImage.GetIntensity(2, 2), 9);
...
{
    cv::Mat1f allOnes = cv::Mat1f::ones(14, 15);
