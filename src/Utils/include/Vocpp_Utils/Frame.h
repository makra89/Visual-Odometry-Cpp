...
public:
	...
	/**
	  ...
	  *
	  * \param[in] in_image image will be moved to frame object !!!
	  * \param[in] in_imgId ID of frame to be created
      */
	Frame(cv::Mat&& in_image, const uint32_t in_imgId);
	...
	/**
      ...
      */
	const cv::Mat& GetImage(const bool grayscale = true)
	{
		if (grayscale) return m_grayImage;
		else return m_image;
	}
	...
	/**
      ...
      */
	cv::Mat GetImageCopy(const bool grayscale = true)
	{
		if (grayscale) return m_grayImage;
		else return m_image;
	}
